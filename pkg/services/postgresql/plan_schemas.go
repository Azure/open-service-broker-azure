...
func generateProvisioningParamsSchema(
	...
	schema := *generateUpdatingParamsSchema(td)
	schema.PropertySchemas["version"] = &service.StringPropertySchema{
		AllowedValues: []string{"9.6"},
		DefaultValue:  "9.6",
	}
	schema.PropertySchemas["tier"] = &service.StringPropertySchema{
		Description:   "Specifies the service tier",
		AllowedValues: []string{td.tier},
		DefaultValue:  td.tier,
	}
	schema.PropertySchemas["hardwareFamily"] = &service.StringPropertySchema{
		Description:   "Specifies the compute generation to use for the DBMS",
		AllowedValues: td.allowedHardware,
		DefaultValue:  "Gen5",
	}
	schema.PropertySchemas["backupRedundancy"] = &service.StringPropertySchema{
		Description:   "Specifies the backup redundancy",
		AllowedValues: td.allowedBackupRedundancy,
		DefaultValue:  "local",
	}
	if includeDBParams {
		schema.PropertySchemas["extensions"] = dbExtensionsSchema
	}
	return schema
}
